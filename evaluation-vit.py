...
def visualize_structure(structure, save_path=None, W=20, H=16):
    ...
    #non_zero_indices = np.argwhere(visual_structure)
    
    # Get the bounds of the non-zero elements
    #topmost_index = np.min(non_zero_indices[:, 0])
    #bottommost_index = np.max(non_zero_indices[:, 0])
    #leftmost_index = np.min(non_zero_indices[:, 1])
    #rightmost_index = np.max(non_zero_indices[:, 1])
    
    # Trim the array
    #trimmed_structure = visual_structure[topmost_index:bottommost_index + 1, leftmost_index:rightmost_index + 1]
        
    
    height, width = visual_structure.shape

    plt.figure(figsize=(width, height))
    plt.imshow(visual_structure, cmap="Greys", origin="lower")
    plt.axis('off')
    if save_path:
